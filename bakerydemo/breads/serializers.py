...
class BreadIndexPageInitialBreadSerializer(Serializer):
    total_count = SerializerMethodField()
    items = SerializerMethodField()

    def get_items(self, qs):
        breads = qs[:6]
        serializer = FeaturedBreadPageSerializer(many=True, instance=breads)
        return serializer.data

    def get_total_count(self, qs):
        return qs.count()
